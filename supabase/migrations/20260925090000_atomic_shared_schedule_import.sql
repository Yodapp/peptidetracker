-- Import every item from a shared schedule in one transaction. Keep peptide IDs
-- referenced by historical logs and never rewrite an old vial's preparation.
...
begin
  ...
  for item in select value from jsonb_array_elements(shared.items) loop
    ...
    if match_count > 1 then
      raise exception 'Flera peptider heter %. Inget importerades.', item->>'name';
    end if;
  end loop;
  ...
  for item in select value from jsonb_array_elements(shared.items) loop
    ...
    select id into target_peptide_id from public.peptides where user_id = account_id
      and lower(btrim(name)) = normalized_name;
