...
function TodayView({ store, update, openCalendar }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openCalendar: () => void }) {
  ...
  return <>
    <PageHeader eyebrow="Peptime" title="Idag" action={<div className="rounded-full bg-accent px-3 py-2 text-sm font-semibold text-accent-foreground"><span className="mr-1">{streak}</span> dagar</div>} />
    ...
    <div className="space-y-5">{groups.map((target,index) => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const previous=groups[index-1]; const showHeading=!previous||previous.carryover!==carryover||resolvedSchedule(previous.items[0],store.mixGroups).slot!==schedule.slot; const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const totalDoseMcg=items.reduce((sum,p)=>sum+p.doseMcg,0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massDoseLine=`${massN(massUnit==="mg"?totalDoseMcg/1000:totalDoseMcg)} ${massUnit}`; const targetKey=`${scheduledDate}:${items.map(i=>i.id).join(",")}`; const selectedSite=siteSelections[targetKey]; const expired=items.some(item=>(vialDaysLeft(item)??1)<=0); const vialLife=items.map(item=>({item,days:vialDaysLeft(item)})).filter(value=>value.days!==null); const lowInventory=items.map(item=>({item,days:inventoryDaysLeft(item,store)})).filter(value=>value.days!==null&&value.days<=15); return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5">
        ...
        <p className="mb-4 flex items-center gap-2 text-sm text-muted-foreground"><FlaskConical className="size-4" /> Kvar: {items.map(i=>`${peptideCode(i)} ${n(i.remainingMg)} mg`).join(" · ")}</p>
        ...
        {vialLife.some(({days})=>days!==null&&days<=0)&&<div className="mb-4 flex flex-wrap gap-2" aria-label="Vialens användningstid">{vialLife.filter(({days})=>days!==null&&days<=0).map(({item})=><span key={item.id} className="rounded-xl bg-destructive/10 px-3 py-2 text-sm text-destructive">{peptideCode(item)} · angiven gräns passerad</span>)}</div>}
        ...
        <div className="mt-2 grid grid-cols-2 gap-2"><Button className="h-12 rounded-xl text-[15px] text-muted-foreground" variant="ghost" onClick={() => {haptic();saveLogs(target,"skipped")}}>Hoppa över</Button><Button className="h-12 rounded-xl text-[15px] text-muted-foreground" variant="ghost" onClick={() => {setAdjustTarget(target);setAdjustDose(first.doseMcg)}}>Justera dos</Button></div>
      </Card>
    </section>})}</div>
