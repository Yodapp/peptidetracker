  ...
  return <>
    ...
    <div className="space-y-7">{groups.map(target => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massDose=(peptide:Peptide)=>`${massN(massUnit==="mg"?peptide.doseMcg/1000:peptide.doseMcg)} ${massUnit}`; const massDoseLine=isMix?items.map(massDose).join(" + "):massDose(first); const expired = first.reconstitutedAt ? (APP_OPENED_AT - new Date(first.reconstitutedAt).getTime()) / 86400000 > first.beyondUseDays : false; return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        ...
        <div className="mb-4 rounded-2xl bg-muted/70 px-4 py-4 tabular-nums"><div className="flex items-center justify-between gap-4"><span className="flex items-center gap-2 text-sm text-muted-foreground"><Syringe className="size-4" /> {isMix ? "En spruta" : "U-100"}</span><strong className="text-2xl font-medium leading-none">{n(totalIu)} IU</strong></div>{isMix?<div className="mt-3 space-y-2 border-t border-border/60 pt-3">{items.map(i=><div key={i.id} className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3 text-sm"><span className="truncate text-muted-foreground">{i.shortCode}</span><span className="font-medium text-foreground">{n(syringeUnits(i.doseMcg,i.vialMg,i.waterMl))} IU</span><span className="min-w-20 text-right font-medium text-foreground/80">{massDose(i)}</span></div>)}</div>:<p className="mt-2 text-right text-base font-medium leading-none text-foreground/85">{massDoseLine}</p>}</div>
