...
import { displayLogDate, effectiveLogDate, stockholmHour } from "@/lib/log-day";
...
function TodayView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  const today = effectiveLogDate(new Date(), store.settings.dayBoundaryHour);
  ...
  const loggedIds = new Set(store.logs.filter(l => effectiveLogDate(l.takenAt, store.settings.dayBoundaryHour) === today).map(l => l.peptideId));
  ...
  const groups = (() => {
    ...
    due.forEach(p => { const group = p.mixGroupId?.trim().toLocaleLowerCase("sv-SE"); const key = group ? `${p.slot}-${p.time}-${group}` : p.id; map.set(key, [...(map.get(key) ?? []), p]); });
    return [...map.values()];
  })();
  ...
  const afterMidnight = stockholmHour() < store.settings.dayBoundaryHour;
  ...
  return <>
    ...
    {afterMidnight && <div className="-mt-5 mb-7 rounded-xl bg-accent/40 px-3 py-2.5 text-xs text-accent-foreground">Efter midnatt · loggas fortfarande på föregående dag fram till {String(store.settings.dayBoundaryHour).padStart(2,"0")}:00.</div>}
    ...
    {done.length > 0 && <section className="mt-8"><h2 className="mb-3 text-sm font-medium text-muted-foreground">Klart</h2><Card className="divide-y divide-border">{done.map(p => { const log = store.logs.find(l=>l.peptideId===p.id && effectiveLogDate(l.takenAt,store.settings.dayBoundaryHour)===today); return <div key={p.id} className="flex min-h-14 items-center gap-3 px-4 text-sm"><CheckCircle2 className="size-5 text-primary"/><span className="flex-1 text-muted-foreground">{p.name}</span><span className="tabular-nums text-muted-foreground">{log?.status === "skipped" ? "Överhoppad" : `${n(log?.computedIu ?? 0)} IU`}</span></div>})}</Card></section>}
...
function LogView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const logs=store.logs.filter(l=>(!query||l.peptideName.toLowerCase().includes(query.toLowerCase()))&&(!date||effectiveLogDate(l.takenAt,store.settings.dayBoundaryHour)===date)).sort((a,b)=>b.takenAt.localeCompare(a.takenAt));
  return <><PageHeader eyebrow="Historik" title="Logg"/><div className="mb-6 grid grid-cols-[1fr_142px] gap-2"><div className="relative"><Search className="absolute left-3 top-3.5 size-4 text-muted-foreground"/><Input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Peptid" className="h-11 pl-9"/></div><Input type="date" value={date} onChange={e=>setDate(e.target.value)} className="h-11"/></div>{logs.length===0?<Card className="p-7 text-center"><History className="mx-auto size-7 text-muted-foreground"/><p className="mt-4 font-medium">Ingen historik ännu</p><p className="mt-2 text-sm text-muted-foreground">Loggade och överhoppade doser visas här.</p></Card>:<div className="space-y-3">{logs.map(log=><Card key={log.id} className="p-4"><div className="flex items-start gap-3"><div className={`mt-0.5 grid size-9 place-items-center rounded-full ${log.status==="taken"?"bg-accent text-accent-foreground":"bg-muted text-muted-foreground"}`}>{log.status==="taken"?<Check className="size-4"/>:<X className="size-4"/>}</div><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><p className="font-medium">{log.peptideName}</p><span className="text-xs tabular-nums text-muted-foreground">{new Intl.DateTimeFormat("sv-SE",{timeZone:"Europe/Stockholm",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}).format(new Date(log.takenAt))}</span></div><p className="mt-1 text-sm text-muted-foreground">{log.status==="taken"?`${n(log.actualDose)} ${log.unit} · ${n(log.computedIu)} IU${log.site?` · ${log.site}`:""}`:"Överhoppad"}</p></div><Button variant="ghost" size="icon" aria-label="Redigera logg" onClick={()=>setEdit(log)}><MoreHorizontal/></Button></div></Card>)}</div>}
...
function PeptidesView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const existingGroups = [...new Map(store.peptides.flatMap(peptide => peptide.mixGroupId?.trim() ? [[peptide.mixGroupId.trim().toLocaleLowerCase("sv-SE"), peptide.mixGroupId.trim()] as const] : [])).values()];
  const save=()=>{const typedGroup=draft.mixGroupId?.trim();const canonicalGroup=existingGroups.find(group=>group.toLocaleLowerCase("sv-SE")===typedGroup?.toLocaleLowerCase("sv-SE"))??typedGroup;const item={...draft,id:draft.id||uid(),shortCode:draft.shortCode||draft.name.slice(0,3),remainingMg:draft.id?draft.remainingMg:draft.vialMg,mixGroupId:canonicalGroup||undefined};update(s=>({...s,peptides:draft.id?s.peptides.map(p=>p.id===draft.id?item:p):[...s.peptides,item]}));setEditing(null);setAdding(false)};
...
function SettingsView({ store, update, syncState }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; syncState: "local" | "syncing" | "synced" | "error" }) {
  ...
  return <><PageHeader eyebrow="Peptime" title="Inställningar"/><div className="space-y-5"><Card className="divide-y divide-border"><SettingRow label="Spruta"><select className="bg-transparent text-right text-sm" value={store.settings.syringe} onChange={e=>update(s=>({...s,settings:{...s.settings,syringe:e.target.value as PeptimeStore["settings"]["syringe"]}}))}><option>U-100 1 ml</option><option>U-100 0.5 ml</option></select></SettingRow><SettingRow label="Tidszon"><span className="text-sm text-muted-foreground">Europe/Stockholm</span></SettingRow><SettingRow label="Ny loggdag"><select aria-label="När en ny loggdag börjar" className="bg-transparent text-right text-sm" value={store.settings.dayBoundaryHour} onChange={e=>update(s=>({...s,settings:{...s.settings,dayBoundaryHour:Number(e.target.value)}}))}>{[0,2,3,4,5,6].map(hour=><option key={hour} value={hour}>{String(hour).padStart(2,"0")}:00</option>)}</select></SettingRow><div className="px-4 pb-3 text-xs leading-5 text-muted-foreground">Doser efter midnatt räknas till föregående dag fram till denna tid.</div><SettingRow label="Språk"><select className="bg-transparent text-sm" value={store.settings.language} onChange={e=>update(s=>({...s,settings:{...s.settings,language:e.target.value as "sv"|"en"}}))}><option value="sv">Svenska</option><option value="en">English</option></select></SettingRow><SettingRow label="Mörkt tema"><Switch checked={store.settings.theme==="dark"} onCheckedChange={setTheme}/></SettingRow></Card><section><h2 className="mb-3 text-sm font-medium">Export</h2><Card className="grid grid-cols-2 gap-2 p-3"><Button variant="outline" className="h-12" onClick={()=>download("csv")}><Download/> CSV</Button><Button variant="outline" className="h-12" onClick={()=>download("json")}><Download/> JSON</Button></Card></section><Card className="p-5"><div className="flex gap-3"><ShieldCheck className="size-5 shrink-0 text-accent-foreground"/><div><p className="font-medium">Om Peptime</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{disclaimer}</p><p className={`mt-3 text-xs ${syncState==="error"?"text-destructive":"text-muted-foreground"}`}>{syncText}</p></div></div></Card><form action="/auth/signout" method="post"><Button type="submit" variant="outline" className="h-12 w-full">Logga ut</Button></form></div></>;
}
...
export function PeptimeApp() {
  ...
  return <main className="mx-auto min-h-dvh w-full max-w-[500px] bg-background px-5 pb-28 pt-7 shadow-2xl shadow-black/10 sm:px-6">{view==="today"&&<TodayView store={store} update={update}/>} {view==="log"&&<LogView store={store} update={update}/>} {view==="peptides"&&<PeptidesView store={store} update={update}/>} {view==="calendar"&&<CalendarView store={store}/>} {view==="settings"&&<SettingsView store={store} update={update} syncState={syncState}/>}<BottomNav view={view} setView={setView}/></main>;
}
