  ...
  return <>
    ...
    <div className="space-y-7">{groups.map(target => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const totalDoseMcg=items.reduce((sum,p)=>sum+p.doseMcg,0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massDoseLine=`${massN(massUnit==="mg"?totalDoseMcg/1000:totalDoseMcg)} ${massUnit}`; const targetKey=`${scheduledDate}:${items.map(i=>i.id).join(",")}`; const selectedSite=siteSelections[targetKey]; const expired=items.some(item=>(vialDaysLeft(item)??1)<=0); const vialLife=items.map(item=>({item,days:vialDaysLeft(item)})).filter(value=>value.days!==null); return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        <div className="mb-5 grid grid-cols-[44px_minmax(0,1fr)_auto] items-start gap-3"><ChemicalBadge items={items}/><p className="min-w-0 pt-0.5 text-[17px] font-medium leading-snug">{items.map(i=>i.name).join(" + ")}</p><span className={`rounded-full border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wide ${late?"border-destructive/20 bg-destructive/10 text-destructive":"border-border bg-muted/60 text-muted-foreground"}`}>{late?"Sen":"Kommande"}</span></div>
