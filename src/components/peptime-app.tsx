...
function TodayView({ store, update, openCalendar }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openCalendar: () => void }) {
  ...
  const saveLogs = (target: LogTarget, status: "taken" | "skipped", site?: string, actual?: number) => {
    ...
    if (completesToday && !hasCheckinData) {
  ...
  return <>
    ...
    {showCheckinPrompt && !checkinOpen && <section ref={checkinRef} className="mt-7 scroll-mt-24"><Card className="overflow-hidden border-primary/30"><button type="button" onClick={openCheckin} className="flex min-h-[100px] w-full items-center gap-4 p-5 text-left"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-accent text-accent-foreground"><Sparkles className="size-5"/></span><span className="min-w-0 flex-1"><span className="block text-[17px] font-semibold">Logga dagens mående</span><span className="mt-1 block text-[13px] leading-5 text-muted-foreground">Sömn, trötthet, aktivitet och anteckning</span></span><ChevronRight className="size-5 shrink-0 text-primary"/></button></Card></section>}
