...
function BottomNav({ view, setView }: { view: string; setView: (view: string) => void }) {
  ...
  const activeView = view === "planner" ? "peptides" : view;
...
function PeptidesView({ store, update, openPlanner }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openPlanner: () => void }) {
...
export function PeptimeApp({ userEmail }: { userEmail?: string }) {
  ...
  return <main className="mx-auto min-h-dvh w-full max-w-[500px] bg-background px-5 pb-28 shadow-2xl shadow-black/10 sm:px-6">{view==="today"&&<TodayView store={store} update={update}/>} {view==="log"&&<LogView store={store} update={update}/>} {view==="peptides"&&<PeptidesView store={store} update={update} openPlanner={()=>setView("planner")}/>} {view==="planner"&&<PurchasePlanner peptides={store.peptides} plans={store.purchasePlans} onChange={purchasePlans=>update(s=>({...s,purchasePlans}))} onBack={()=>setView("peptides")}/>} {view==="calendar"&&<CalendarView store={store}/>} {view==="settings"&&<SettingsView store={store} update={update} syncState={syncState} retrySync={retrySync} userEmail={userEmail}/>}<BottomNav view={view} setView={setView}/></main>;
}
