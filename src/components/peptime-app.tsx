  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      try {
        ...
        setSyncError(recoveryMode.current ? "Återställningskopior finns kvar. Automatisk kontosynk är pausad för att skydda dem." : null);
...
function TodayView({ store, update, openCalendar }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openCalendar: () => void }) {
  ...
  const saveLogs = (target: LogTarget, status: "taken" | "skipped", site?: string, actualDoses?: Record<string, number>) => {
    ...
    const newLogs: DoseLog[] = items.map(p => {
      ...
      return { id: uid(), peptideId: p.id, peptideName: p.name, plannedDose: p.doseMcg, actualDose, unit: "mcg", computedIu: syringeUnits(actualDose, p.vialMg, p.waterMl), slot: resolvedSchedule(p,store.mixGroups).slot, takenAt: now, scheduledDate, status, site, mixGroupId: p.mixGroupId, note: "" };
    });
