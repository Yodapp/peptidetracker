...
function TodayView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const scheduledToday = store.peptides.filter(p => isDueOn(p, store, today));
  const due = scheduledToday.filter(p => !loggedIds.has(p.id));
  const done = store.peptides.filter(p => !p.archived && loggedIds.has(p.id));
  const groups = (() => {
    ...
    return [...map.values()].sort((a,b) => {const aSchedule=resolvedSchedule(a[0],store.mixGroups),bSchedule=resolvedSchedule(b[0],store.mixGroups);return (aSchedule.frequency==="as_needed"?"99:99":aSchedule.time).localeCompare(bSchedule.frequency==="as_needed"?"99:99":bSchedule.time)});
  })();
  const saveLogs = (items: Peptide[], status: "taken" | "skipped", site?: string, actual?: number) => {
    const now = new Date().toISOString();
    const newLogs: DoseLog[] = items.map(p => ({ id: uid(), peptideId: p.id, peptideName: p.name, plannedDose: p.doseMcg, actualDose: actual ?? p.doseMcg, unit: "mcg", computedIu: syringeUnits(actual ?? p.doseMcg, p.vialMg, p.waterMl), slot: resolvedSchedule(p,store.mixGroups).slot, takenAt: now, status, site, mixGroupId: p.mixGroupId, note: "" }));
  ...
  return <>
    ...
    <div className="space-y-7">{groups.map(items => { const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=schedule.frequency!=="as_needed"&&(afterMidnight||nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const isMix = items.length > 1; const expired = first.reconstitutedAt ? (APP_OPENED_AT - new Date(first.reconstitutedAt).getTime()) / 86400000 > first.beyondUseDays : false; return <section key={items.map(i=>i.id).join("-")}>
      <div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-medium">{slotNames[schedule.slot]}</h2><span className="flex items-center gap-1.5 text-xs tabular-nums text-muted-foreground"><Clock3 className="size-3.5" /> {schedule.frequency==="as_needed"?"Manuell":schedule.time}</span></div>
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        <div className="mb-5 flex items-start gap-3"><div className="grid size-11 shrink-0 place-items-center rounded-xl bg-accent text-xs font-semibold text-accent-foreground">{isMix ? items.map(i=>i.shortCode[0]).join("+") : first.shortCode}</div><div className="min-w-0 flex-1"><p className="font-medium">{items.map(i=>i.name).join(" + ")}</p><p className="mt-1 text-sm text-muted-foreground">{items.map(i=><span key={i.id} className="mr-2 text-foreground">{n(i.doseMcg)} mcg</span>)}</p></div><span className={`rounded-full px-2.5 py-1 text-[11px] ${late?"bg-destructive/10 text-destructive":"bg-muted text-muted-foreground"}`}>{late?"Sen":"Kommande"}</span></div>
...
function PeptidesView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const addToday=(peptide:Peptide)=>{const date=effectiveLogDate(new Date(),store.settings.dayBoundaryHour);const key=`${date}:${scheduleTargetKey(peptide)}`;update(s=>({...s,todayAdditions:[...new Set([...s.todayAdditions,key])]}))};
