  ...
  return <>
    ...
    <div className="space-y-7">{groups.map(target => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const totalDoseMcg=items.reduce((sum,p)=>sum+p.doseMcg,0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massDoseLine=`${massN(massUnit==="mg"?totalDoseMcg/1000:totalDoseMcg)} ${massUnit}`; const expired = first.reconstitutedAt ? (APP_OPENED_AT - new Date(first.reconstitutedAt).getTime()) / 86400000 > first.beyondUseDays : false; return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        <div className="mb-5 grid grid-cols-[44px_minmax(0,1fr)_auto] items-start gap-3"><div className="grid size-11 place-items-center rounded-xl bg-accent text-xs font-semibold text-accent-foreground">{isMix ? items.map(i=>i.shortCode[0]).join("+") : first.shortCode}</div><p className="min-w-0 pt-0.5 text-[17px] font-medium leading-snug">{items.map(i=>i.name).join(" + ")}</p><span className={`rounded-full px-2.5 py-1 text-[11px] ${late?"bg-destructive/10 text-destructive":"bg-muted text-muted-foreground"}`}>{late?"Sen":"Kommande"}</span></div>
        ...
        {isMix?<div className="mb-4 overflow-hidden rounded-2xl bg-muted/70 tabular-nums"><div className="flex items-center gap-2 px-4 pb-2 pt-4 text-xs font-medium uppercase tracking-[.12em] text-muted-foreground"><Syringe className="size-4" /> Dra upp från respektive vial</div><div className="divide-y divide-border/60 px-4">{items.map(i=><div key={i.id} className="grid min-h-[68px] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-3"><span className="min-w-0 text-[15px] font-medium leading-snug">{i.name}</span><span className="shrink-0 text-right"><strong className="block text-2xl font-semibold leading-none tracking-tight">{n(syringeUnits(i.doseMcg,i.vialMg,i.waterMl))} IU</strong><span className="mt-1.5 block text-sm font-medium leading-none text-foreground/70">{massN(massUnit==="mg"?i.doseMcg/1000:i.doseMcg)} {massUnit}</span></span></div>)}</div><div className="flex items-center justify-between gap-4 border-t border-border bg-foreground/[.035] px-4 py-3.5"><span className="text-sm font-medium">Totalt i sprutan</span><span className="shrink-0 text-right"><strong className="block text-2xl font-semibold leading-none tracking-tight">{n(totalIu)} IU</strong><span className="mt-1.5 block text-sm font-medium leading-none text-foreground/70">{massDoseLine}</span></span></div></div>:<div className="mb-4 flex items-center justify-between gap-4 rounded-2xl bg-muted/70 px-4 py-4 tabular-nums"><span className="flex items-center gap-2 text-sm text-muted-foreground"><Syringe className="size-4" /> U-100</span><span className="text-right"><strong className="block text-2xl font-medium leading-none">{n(totalIu)} IU</strong><span className="mt-2 block text-base font-medium leading-none text-foreground/85">{massDoseLine}</span></span></div>}
