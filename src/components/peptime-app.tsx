...
import {
  Archive, BarChart3, Bell, CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, Clock3,
...
import { syringeCapacity, syringeUnits, type DailyNote, type DailyTagId, type DoseLog, type MixGroupSchedule, type Peptide, type PeptimeStore, type Schedule, type Slot, type WellbeingMetric } from "@/lib/types";
...
function WellbeingScale({ scale, value, onChange }: { scale: typeof wellbeingScales[number]; value?: number; onChange: (value: number) => void }) {
  return <fieldset className="rounded-2xl border border-border bg-background/45 p-4"><legend className="sr-only">{scale.title}</legend><div className="mb-3"><p className="font-medium">{scale.title}</p><p className="mt-0.5 text-xs text-muted-foreground">{scale.description}</p></div><div className="grid grid-cols-5 gap-2">{[1,2,3,4,5].map(step => <button type="button" key={step} aria-label={`${scale.title}: ${step} av 5`} aria-pressed={value === step} onClick={() => { haptic(); onChange(step); }} className={`grid min-h-11 place-items-center rounded-xl border font-mono text-sm transition-colors ${value === step ? "border-primary bg-accent font-semibold text-accent-foreground" : "border-border bg-card text-muted-foreground"}`}>{step}</button>)}</div><div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>{scale.low}</span><span>{scale.high}</span></div></fieldset>;
}
...
function FastedBadge() {
  return <span className="inline-flex min-h-6 items-center rounded-full border border-amber-700/20 bg-amber-500/10 px-2 text-xs font-medium text-amber-800 dark:border-amber-300/20 dark:text-amber-200">Fastande</span>;
}
...
function SyringeDrawBar({ items }: { items: Peptide[] }) {
  ...
  return <div className="px-4 pb-3 pt-3" aria-label={`Sprutvisualisering, totalt ${n(total)} IU`}>
    ...
    <div className="relative mt-1.5 h-4 font-mono text-[10px] text-muted-foreground"><span className="absolute left-0">0</span>{stops.map((stop,index)=><span key={`${items[index].id}-stop`} style={{ left: `${(stop / total) * 100}%` }} className={`absolute ${index === stops.length - 1 ? "-translate-x-full" : "-translate-x-1/2"}`}>{n(stop)}</span>)}</div>
  </div>;
}
...
function PageHeader({ eyebrow, title, action }: { eyebrow?: string; title: string; action?: React.ReactNode }) {
  return <header className="sticky top-0 z-20 -mx-5 mb-7 flex min-h-[86px] items-end justify-between gap-4 border-b border-border/40 bg-background/80 px-5 pb-3 pt-6 backdrop-blur-[20px] sm:-mx-6 sm:px-6"><div>{eyebrow && <p className="mb-1 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-accent-foreground">{eyebrow}</p>}<h1 className="text-[30px] font-medium tracking-[-0.045em]">{title}</h1></div>{action}</header>;
}

function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-[20px] border border-border bg-card shadow-[0_16px_50px_rgba(0,0,0,.24)] ${className}`}>{children}</div>;
}

function BottomNav({ view, setView }: { view: string; setView: (view: string) => void }) {
  const items = [[House, "today", "Idag"], [History, "log", "Logg"], [FlaskConical, "peptides", "Peptider"], [BarChart3, "insights", "Insikter"], [CalendarDays, "calendar", "Kalender"], [Settings, "settings", "Inställningar"]] as const;
  const activeView = view === "planner" || view === "peptide-insights" ? "peptides" : view;
  return <nav aria-label="Huvudnavigering" className="fixed inset-x-0 bottom-0 z-30 mx-auto flex h-[calc(76px+env(safe-area-inset-bottom))] max-w-[500px] items-start justify-around border-t border-border bg-background/80 px-1 pt-3 shadow-[0_-16px_50px_rgba(0,0,0,.32)] backdrop-blur-[20px] dark:bg-black/70">
    {items.map(([Icon, key, label]) => <button key={key} aria-current={activeView === key ? "page" : undefined} onClick={() => setView(key)} className={`flex min-h-12 min-w-[48px] flex-col items-center justify-center gap-1 text-[10px] transition-colors ${activeView === key ? "text-foreground" : "text-muted-foreground"}`}><Icon className="size-[19px]" strokeWidth={activeView === key ? 2.3 : 1.7} />{label}</button>)}
  </nav>;
}
...
function Onboarding({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-background p-5"><div className="w-full max-w-[430px]">
    ...
    {step === 3 && <><div className="mb-6 grid size-14 place-items-center rounded-2xl bg-accent text-accent-foreground"><Sparkles className="size-7" /></div><h1 className="text-3xl font-medium tracking-[-.04em]">Redo när du är</h1><p className="mt-4 leading-7 text-muted-foreground">Öppna appen och tryck Ta dos. Injektionsplats kan väljas direkt på kortet när du vill logga den.</p><Card className="mt-7 p-5"><p className="text-sm text-muted-foreground">Standard</p><p className="mt-2">{store.settings.syringe} · Europe/Stockholm · Mörkt tema</p></Card></>}
...
function TodayView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const hasCheckinData = Boolean(noteEntry.note.trim() || noteEntry.tags.length || wellbeingScales.some(scale => noteEntry[scale.key] !== undefined));
  const showCheckinPrompt = scheduledToday.length > 0 && groups.every(group => group.carryover) && !hasCheckinData;
  ...
  return <>
    <PageHeader eyebrow="Peptime" title="Idag" action={<div className="rounded-full border border-border bg-card px-3 py-2 font-mono text-[10px] uppercase tracking-wide text-muted-foreground"><span className="mr-1 text-foreground">{streak}</span> dagar loggade</div>} />
    ...
    <div className="space-y-7">{groups.map(target => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const totalDoseMcg=items.reduce((sum,p)=>sum+p.doseMcg,0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massDoseLine=`${massN(massUnit==="mg"?totalDoseMcg/1000:totalDoseMcg)} ${massUnit}`; const targetKey=`${scheduledDate}:${items.map(i=>i.id).join(",")}`; const selectedSite=siteSelections[targetKey]; const expired=items.some(item=>(vialDaysLeft(item)??1)<=0); const vialLife=items.map(item=>({item,days:vialDaysLeft(item)})).filter(value=>value.days!==null); const lowInventory=items.map(item=>({item,days:inventoryDaysLeft(item,store)})).filter(value=>value.days!==null&&value.days<=15); return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        <div className="mb-5 grid grid-cols-[44px_minmax(0,1fr)_auto] items-start gap-3"><ChemicalBadge items={items}/><div className="min-w-0 pt-0.5"><p className="text-[17px] font-medium leading-snug">{items.map(i=>i.name).join(" + ")}</p>{!isMix&&first.fasted&&<div className="mt-2"><FastedBadge/></div>}</div><span className={`rounded-full border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wide ${late?"border-destructive/20 bg-destructive/10 text-destructive":"border-border bg-muted/60 text-muted-foreground"}`}>{late?"Sen":"Kommande"}</span></div>
        ...
        {isMix?<div className="mb-4 overflow-hidden rounded-2xl border border-border bg-muted/40 tabular-nums dark:bg-black/25"><div className="flex items-center gap-2 px-4 pb-2 pt-3 font-mono text-[10px] font-medium uppercase tracking-[.12em] text-muted-foreground"><Syringe className="size-3.5" /> Dra upp från respektive vial</div><div className="grid grid-cols-2 border-y border-border">{items.map((i,index)=><div key={i.id} className={`min-w-0 px-4 py-3 ${index%2?"border-l border-border":""}`}><div className="min-h-[68px]"><p className="text-[15px] font-medium leading-tight">{i.name}</p>{i.fasted&&<div className="mt-2"><FastedBadge/></div>}</div><strong className="mt-2 block font-mono text-2xl font-semibold leading-none tracking-tight">{n(syringeUnits(i.doseMcg,i.vialMg,i.waterMl))} IU</strong><span className="mt-1.5 block text-base font-medium leading-none text-foreground/70">{massN(massUnit==="mg"?i.doseMcg/1000:i.doseMcg)} {massUnit}</span></div>)}</div><SyringeDrawBar items={items}/><div className="flex min-h-14 items-center justify-between gap-3 border-t border-border bg-muted/40 px-4 py-2.5 dark:bg-white/[.025]"><span className="text-[15px] font-semibold">Totalt i sprutan</span><span className="shrink-0 text-right"><strong className="block font-mono text-2xl font-semibold leading-none tracking-tight">{n(totalIu)} IU</strong><span className="mt-1.5 block text-base font-medium leading-none text-foreground/75">{massDoseLine}</span></span></div></div>:<div className="mb-4 flex items-center justify-between gap-4 rounded-2xl border border-border bg-muted/40 px-4 py-4 tabular-nums dark:bg-black/25"><span className="flex items-center gap-2 text-sm text-muted-foreground"><Syringe className="size-4" /> U-100</span><span className="text-right"><strong className="block font-mono text-[28px] font-semibold leading-none">{n(totalIu)} IU</strong><span className="mt-2 block text-base font-medium leading-none text-foreground/75">{massDoseLine}</span></span></div>}
        {totalIu > syringeCapacity(store.settings.syringe) && <div className="mb-4 flex gap-2 rounded-xl bg-destructive/10 p-3 text-xs text-destructive"><TriangleAlert className="size-4 shrink-0"/>Dosen är {n(totalIu)} IU och ryms inte i din valda {store.settings.syringe}-spruta ({syringeCapacity(store.settings.syringe)} IU).</div>}
        <p className="mb-3 flex items-center gap-2 text-xs text-muted-foreground"><FlaskConical className="size-3.5" /> Kvar: {items.map(i=>`${peptideCode(i)} ${n(i.remainingMg)} mg`).join(" · ")}</p>
        {lowInventory.length>0&&<div className="mb-4 flex flex-wrap gap-1.5" aria-label="Lågt lager">{lowInventory.map(({item,days})=><span key={item.id} className="inline-flex min-h-7 items-center gap-1.5 rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 text-xs font-medium text-amber-800 dark:text-amber-200"><TriangleAlert className="size-3.5"/>{peptideCode(item)} · {days===0?"slut":"ca "+days+" dagar kvar"}</span>)}</div>}
        {vialLife.length>0&&<div className="mb-4 flex flex-wrap gap-1.5" aria-label="Tid kvar enligt angiven vialgräns">{vialLife.map(({item,days})=><span key={item.id} className={`rounded-full border px-2.5 py-1 font-mono text-[10px] ${days!==null&&days<=0?"border-destructive/20 bg-destructive/10 text-destructive":"border-border bg-muted/50 text-muted-foreground"}`}>{peptideCode(item)} · {days!==null&&days>0?`${days} d kvar enligt din gräns`:"angiven gräns passerad"}</span>)}</div>}
        {first.route==="subcutaneous"&&first.sites.length>0&&<div className="mb-4"><div className="mb-2 flex items-center justify-between"><p className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Plats <span className="normal-case tracking-normal">· valfritt</span></p>{first.lastSite&&<span className="text-[10px] text-muted-foreground">Senast: {first.lastSite}</span>}</div><div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1">{first.sites.map(site=><button type="button" key={site} aria-pressed={selectedSite===site} onClick={()=>{haptic();setSiteSelections(value=>({...value,[targetKey]:value[targetKey]===site?"":site}))}} className={`min-h-11 shrink-0 rounded-full border px-3 text-xs transition-colors ${selectedSite===site?"border-primary bg-accent text-accent-foreground":"border-border bg-background/60 text-muted-foreground dark:bg-black/20"}`}>{site}</button>)}</div></div>}
        <Button className="h-14 w-full rounded-2xl text-base font-semibold shadow-[0_10px_30px_rgba(114,183,170,.16)]" onClick={() => {haptic();saveLogs(target,"taken",selectedSite||undefined)}}><Check className="size-5" /> Ta dos</Button>
        <div className="mt-2 grid grid-cols-2 gap-2"><Button className="h-11 rounded-xl text-muted-foreground" variant="ghost" onClick={() => {haptic();saveLogs(target,"skipped")}}>Hoppa över</Button><Button className="h-11 rounded-xl text-muted-foreground" variant="ghost" onClick={() => {setAdjustTarget(target);setAdjustDose(first.doseMcg)}}>Justera en gång</Button></div>
      </Card>
    </section>})}</div>
    {done.length > 0 && <section className="mt-8"><h2 className="mb-3 text-sm font-medium text-muted-foreground">Klart</h2><Card className="divide-y divide-border">{done.map(p => { const log = store.logs.find(l=>l.peptideId===p.id && logScheduledDate(l,store.settings.dayBoundaryHour)===today); return <div key={p.id} className="flex min-h-14 items-center gap-3 px-4 py-2 text-sm"><CheckCircle2 className="size-5 text-primary"/><span className="min-w-0 flex-1 text-muted-foreground"><span className="block truncate">{p.name}</span>{p.fasted&&<span className="mt-1 block"><FastedBadge/></span>}</span><span className="tabular-nums text-muted-foreground">{log?.status === "skipped" ? "Överhoppad" : `${n(log?.computedIu ?? 0)} IU`}</span></div>})}</Card></section>}
    {showCheckinPrompt && <Card className="mt-8 border-primary/40 bg-accent/25 p-5"><div className="flex items-start gap-3"><Sparkles className="mt-0.5 size-5 shrink-0 text-accent-foreground"/><div className="flex-1"><p className="font-medium">Dagens doser är klara</p><p className="mt-1 text-sm leading-6 text-muted-foreground">Fånga hur dagen har känts medan du minns den.</p><Button className="mt-4 h-11 rounded-xl" onClick={openCheckin}>Gör kvällskoll</Button></div></div></Card>}
    ...
    <section ref={checkinRef} className="mt-9 scroll-mt-24"><Card className="overflow-hidden"><button type="button" className="flex min-h-16 w-full items-center justify-between gap-4 px-5 text-left" onClick={() => setCheckinOpen(value => !value)} aria-expanded={checkinOpen || hasCheckinData}><span><span className="block font-medium">Dagens kvällskoll</span><span className="mt-1 block text-xs text-muted-foreground">Mående, aktivitet, taggar och anteckning</span></span><span className="text-xs text-muted-foreground">{checkinOpen || hasCheckinData ? "Dölj" : "Fyll i"}</span></button>{(checkinOpen || hasCheckinData) && <div className="space-y-4 border-t border-border p-4">{wellbeingScales.map(scale => <WellbeingScale key={scale.key} scale={scale} value={noteEntry[scale.key]} onChange={value => saveNote({ [scale.key]: value })}/>) }<div><p className="mb-2 text-sm font-medium">Hur har dagen känts?</p><div className="flex flex-wrap gap-2">{dailyTags.map(tag=><button type="button" key={tag.id} aria-pressed={noteEntry.tags.includes(tag.id)} onClick={()=>toggleTag(tag.id)} className={`min-h-11 rounded-full border px-3 text-xs transition-colors ${noteEntry.tags.includes(tag.id)?"border-primary bg-accent text-accent-foreground":"border-border bg-card text-muted-foreground"}`}>{tag.label}</button>)}{store.settings.customDailyTags.map(tag=><span key={tag} className="inline-flex overflow-hidden rounded-full border border-border"><button type="button" aria-pressed={noteEntry.tags.includes(tag)} onClick={()=>toggleTag(tag)} className={`min-h-11 px-3 text-xs ${noteEntry.tags.includes(tag)?"bg-accent text-accent-foreground":"bg-card text-muted-foreground"}`}>{tag}</button><button type="button" onClick={()=>removeCustomTag(tag)} aria-label={`Ta bort taggen ${tag}`} className="min-h-11 border-l border-border px-2 text-muted-foreground"><X className="size-3.5"/></button></span>)}</div><div className="mt-3 flex gap-2"><Input value={customTag} onChange={event=>setCustomTag(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();addCustomTag();}}} maxLength={40} placeholder="Skapa egen tagg" className="h-11"/><Button type="button" variant="outline" className="h-11 shrink-0" onClick={addCustomTag}><Plus/> Lägg till</Button></div></div><div><label htmlFor="daily-note" className="mb-2 block text-sm font-medium">Anteckning <span className="font-normal text-muted-foreground">· valfritt</span></label><Textarea id="daily-note" value={note} onChange={e=>saveNote({ note: e.target.value })} placeholder="Något mer du vill komma ihåg?" className="min-h-28 resize-none rounded-2xl bg-card p-4 text-base"/></div><p className="text-right text-[11px] text-muted-foreground">Sparas automatiskt</p></div>}</Card></section>
...
function PeptidesView({ store, update, openPlanner, openInsights }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openPlanner: () => void; openInsights: (id: string) => void }) {
...
function CalendarView({ store }: { store: PeptimeStore }) {
  ...
  const statusForDate=(date:string)=>{const logs=logsForDate(date);const note=store.dailyNotes.find(value=>value.date===date);return {complete:logs.length>0&&logs.every(log=>log.status==="taken"),skipped:logs.some(log=>log.status==="skipped"),noted:Boolean(note?.note.trim()||note?.tags.length||wellbeingScales.some(scale=>note?.[scale.key]!==undefined))}};
  return <><PageHeader eyebrow="Översikt" title="Kalender"/><Card className="p-4"><div className="mb-5 flex items-center justify-between"><Button variant="ghost" size="icon" onClick={()=>setCursor(new Date(year,month-1,1))}><ChevronLeft/></Button><p className="font-medium capitalize">{new Intl.DateTimeFormat("sv-SE",{month:"long",year:"numeric"}).format(cursor)}</p><Button variant="ghost" size="icon" onClick={()=>setCursor(new Date(year,month+1,1))}><ChevronRight/></Button></div><div className="grid grid-cols-7 text-center font-mono text-[10px] text-muted-foreground">{"M T O T F L S".split(" ").map((d,i)=><span key={i} className="py-2">{d}</span>)}{cells.map((day,i)=>{if(!day)return <span key={i}/>;const date=iso(day);const status=statusForDate(date);return <button key={i} onClick={()=>setSelected(date)} className={`relative mx-auto grid size-11 place-items-center rounded-full border text-sm transition-colors ${selected===date?"border-primary bg-accent text-foreground":"border-transparent"}`}>{day}<span className="absolute bottom-0.5 flex gap-0.5">{status.complete&&<span className="size-1.5 rounded-full bg-primary"/>}{status.noted&&<span className="size-1.5 rounded-full bg-[#7f9fca]"/>}{status.skipped&&<span className="size-1.5 rounded-full bg-zinc-500"/>}</span></button>})}</div><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-t border-border pt-3 text-[10px] text-muted-foreground"><span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-primary"/>Klart</span><span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-[#7f9fca]"/>Anteckning</span><span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-zinc-500"/>Överhoppat</span></div></Card><section className="mt-6"><h2 className="mb-3 font-mono text-xs font-medium">{selected}</h2>{dayLogs.length===0&&!dayNote?.note&&!dayNote?.tags.length&&!wellbeingScales.some(scale=>dayNote?.[scale.key]!==undefined)?<p className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Inga poster denna dag.</p>:<Card className="divide-y divide-border">{dayLogs.map(l=><div key={l.id} className="flex min-h-14 items-center justify-between gap-3 px-4 text-sm"><span>{l.peptideName}</span><span className="text-right text-muted-foreground">{l.status==="taken"?`${n(l.actualDose)} mcg · ${n(l.computedIu)} IU`:"Överhoppad"}<span className="ml-2 tabular-nums">{new Intl.DateTimeFormat("sv-SE",{timeZone:"Europe/Stockholm",hour:"2-digit",minute:"2-digit"}).format(new Date(l.takenAt))}</span></span></div>)}{dayNote&&Boolean(dayNote.note||dayNote.tags.length||wellbeingScales.some(scale=>dayNote[scale.key]!==undefined))&&<div className="p-4"><p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Dagens kvällskoll</p>{wellbeingScales.some(scale=>dayNote[scale.key]!==undefined)&&<div className="mb-3 grid grid-cols-2 gap-2">{wellbeingScales.filter(scale=>dayNote[scale.key]!==undefined).map(scale=><div key={scale.key} className="rounded-xl bg-muted/50 p-2 text-xs"><span className="block text-muted-foreground">{scale.title}</span><strong className="mt-1 block">{dayNote[scale.key]} / 5</strong></div>)}</div>}{dayNote.tags.length>0&&<div className="mb-3 flex flex-wrap gap-1.5">{dayNote.tags.map(tag=><span key={tag} className="rounded-full border border-border bg-muted/50 px-2.5 py-1 text-xs">{tagLabel(tag)}</span>)}</div>}{dayNote.note&&<p className="text-sm leading-6">{dayNote.note}</p>}</div>}</Card>}</section></>;
}
...
export function PeptimeApp({ userEmail }: { userEmail?: string }) {
  ...
  return <main className="mx-auto min-h-dvh w-full max-w-[500px] bg-background px-5 pb-28 shadow-2xl shadow-black/10 sm:px-6">{view==="today"&&<TodayView store={store} update={update}/>} {view==="log"&&<LogView store={store} update={update}/>} {view==="peptides"&&<PeptidesView store={store} update={update} openPlanner={()=>setView("planner")} openInsights={id=>openPeptideInsights(id,"peptides")}/>} {view==="peptide-insights"&&insightPeptideId&&store.peptides.find(peptide=>peptide.id===insightPeptideId)&&<PeptideInsights store={store} peptide={store.peptides.find(peptide=>peptide.id===insightPeptideId)!} onBack={()=>setView(insightReturnView)}/>} {view==="insights"&&<InsightsView store={store} onOpenPeptide={id=>openPeptideInsights(id,"insights")}/>} {view==="planner"&&<PurchasePlanner peptides={store.peptides} plans={store.purchasePlans} onChange={purchasePlans=>update(s=>({...s,purchasePlans}))} onBack={()=>setView("peptides")}/>} {view==="calendar"&&<CalendarView store={store}/>} {view==="settings"&&<SettingsView store={store} update={update} syncState={syncState} retrySync={retrySync} userEmail={userEmail}/>}<BottomNav view={view==="peptide-insights"?insightReturnView:view} setView={setView}/></main>;
}
