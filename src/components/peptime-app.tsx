...
function massPerIu(peptide: Peptide, unit: "mcg" | "mg") {
  const mcg = peptide.waterMl > 0 ? (peptide.vialMg / peptide.waterMl) * 10 : 0;
  return unit === "mg" ? mcg / 1000 : mcg;
}
  ...
  return <>
    ...
    <div className="space-y-7">{groups.map(target => { const {items,carryover,scheduledDate}=target; const first = items[0]; const schedule=resolvedSchedule(first,store.mixGroups); const late=carryover||(schedule.frequency!=="as_needed"&&nowTime>schedule.time); const totalIu = items.reduce((sum,p) => sum + syringeUnits(p.doseMcg,p.vialMg,p.waterMl),0); const isMix = items.length > 1; const massUnit=store.settings.massDisplayUnit; const massComparison=isMix?`${items.map(i=>`${i.shortCode} ${massN(massPerIu(i,massUnit))}`).join(" · ")} ${massUnit}/IU`:`1 IU = ${massN(massPerIu(first,massUnit))} ${massUnit}`; const expired = first.reconstitutedAt ? (APP_OPENED_AT - new Date(first.reconstitutedAt).getTime()) / 86400000 > first.beyondUseDays : false; return <section key={`${scheduledDate}-${items.map(i=>i.id).join("-")}`}>
      ...
      <Card className="overflow-hidden p-5 shadow-[0_12px_36px_rgba(0,0,0,.12)]">
        ...
        <div className="mb-4 flex items-center justify-between gap-3 rounded-2xl bg-muted/70 px-4 py-3.5"><span className="flex shrink-0 items-center gap-2 text-sm text-muted-foreground"><Syringe className="size-4" /> {isMix ? "En spruta" : "U-100"}</span><span className="min-w-0 text-right"><strong className="block text-lg font-medium tabular-nums">{isMix ? `${items.map(i=>`${n(syringeUnits(i.doseMcg,i.vialMg,i.waterMl))} IU`).join(" + ")} → ${n(totalIu)} IU` : `${n(totalIu)} IU`}</strong><span className="mt-0.5 block text-[11px] leading-4 tabular-nums text-muted-foreground">{massComparison}</span></span></div>
