...
import {
  Archive, BarChart3, CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, Clock3,
...
function BottomNav({ view, setView }: { view: string; setView: (view: string) => void }) {
  ...
  return <nav aria-label="Huvudnavigering" className="fixed inset-x-0 bottom-0 z-30 mx-auto flex h-[calc(70px+env(safe-area-inset-bottom))] max-w-[500px] items-start justify-around border-t border-border/70 bg-background/88 px-2 pt-2 backdrop-blur-2xl">
    {items.map(([Icon, key, label]) => <button key={key} aria-current={activeView === key ? "page" : undefined} onClick={() => setView(key)} className={`flex min-h-12 min-w-[56px] flex-col items-center justify-center gap-1 rounded-xl text-[11px] font-medium transition-colors ${activeView === key ? "text-primary" : "text-muted-foreground"}`}><Icon className="size-[20px]" strokeWidth={activeView === key ? 2.4 : 1.8} />{label}</button>)}
  </nav>;
}
...
function TodayView({ store, update, openCalendar }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; openCalendar: () => void }) {
  ...
  return <>
    <PageHeader eyebrow="Peptime" title="Idag" action={<div className="rounded-full bg-accent px-3 py-2 text-xs font-semibold text-accent-foreground"><span className="mr-1">{streak}</span> dagar</div>} />
    <button type="button" onClick={openCalendar} className="-mt-3 mb-7 flex min-h-11 items-center gap-2 rounded-xl px-1 text-[15px] capitalize text-primary"><CalendarDays className="size-4"/>{dateText}<ChevronRight className="size-4"/></button>
    ...
    {showCheckinPrompt && !checkinOpen && <section ref={checkinRef} className="mt-7 scroll-mt-24"><Card className="overflow-hidden border-primary/20"><button type="button" onClick={openCheckin} className="flex min-h-[92px] w-full items-center gap-4 p-5 text-left"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-accent text-accent-foreground"><Sparkles className="size-5"/></span><span className="min-w-0 flex-1"><span className="block text-[17px] font-semibold">Gör dagens kvällskoll</span><span className="mt-1 block text-[13px] leading-5 text-muted-foreground">Mående, aktivitet och anteckning</span></span><ChevronRight className="size-5 shrink-0 text-muted-foreground"/></button></Card></section>}
...
function CalendarView({ store, onBack }: { store: PeptimeStore; onBack: () => void }) {
