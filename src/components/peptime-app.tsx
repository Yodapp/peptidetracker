...
export function PeptimeApp({ userEmail }: { userEmail?: string }) {
  ...
  return <main className="mx-auto min-h-dvh w-full max-w-[500px] bg-background px-5 pb-28 shadow-2xl shadow-black/10 sm:px-6">{view==="today"&&<TodayView store={store} update={update}/>} {view==="log"&&<LogView store={store} update={update}/>} {view==="peptides"&&<PeptidesView store={store} update={update}/>} {view==="calendar"&&<CalendarView store={store}/>} {view==="settings"&&<SettingsView store={store} update={update} syncState={syncState} retrySync={retrySync} userEmail={userEmail}/>}<BottomNav view={view} setView={setView}/></main>;
}
