...
import { syringeUnits, type DailyTagId, type DoseLog, type MixGroupSchedule, type Peptide, type PeptimeStore, type Schedule, type Slot } from "@/lib/types";
...
const dailyTags: { id: DailyTagId; label: string }[] = [
  { id: "great_sleep", label: "😴 Sov bra" },
  { id: "flushing", label: "🔥 Rodnad" },
  { id: "high_energy", label: "⚡ Hög energi" },
...
function TodayView({ store, update }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>> }) {
  ...
  const noteEntry = store.dailyNotes.find(v => v.date === today) ?? { date: today, note: "", tags: [] };
  ...
  const saveNote = (value: string, tags = noteEntry.tags) => update(s => ({...s, dailyNotes: [...s.dailyNotes.filter(v => v.date !== today), {date: today, note: value, tags}]}));
  const toggleTag = (tag: DailyTagId) => { haptic(); saveNote(note, noteEntry.tags.includes(tag) ? noteEntry.tags.filter(value => value !== tag) : [...noteEntry.tags, tag]); };
  return <>
    ...
    <section className="mt-9"><label htmlFor="daily-note" className="mb-3 block text-sm font-medium">Dagens anteckning</label><Textarea id="daily-note" value={note} onChange={e=>saveNote(e.target.value)} placeholder="Sömn, energi, humör, biverkningar…" className="min-h-28 resize-none rounded-2xl bg-card p-4 text-base"/><div className="mt-3 flex flex-wrap gap-2">{dailyTags.map(tag=><button type="button" key={tag.id} aria-pressed={noteEntry.tags.includes(tag.id)} onClick={()=>toggleTag(tag.id)} className={`min-h-11 rounded-full border px-3 text-xs transition-colors ${noteEntry.tags.includes(tag.id)?"border-primary bg-accent text-accent-foreground":"border-border bg-card text-muted-foreground"}`}>{tag.label}</button>)}</div><p className="mt-2 text-right text-[11px] text-muted-foreground">Sparas automatiskt</p></section>
