...
import { Download, Plus, Save, ShoppingCart, Trash2 } from "lucide-react";
...
export function PurchasePlanner({ peptides, plans, onChange, onBack }: { peptides: Peptide[]; plans: PurchasePlan[]; onChange: (plans: PurchasePlan[]) => void; onBack: () => void }) {
  ...
  return <>
    <PageHeader title="Inköpsplan" back={{ label: "Peptider", onClick: onBack }}/>
    ...
    <div className="mb-5 grid grid-cols-[1fr_auto] gap-2"><select aria-label="Ladda sparad plan" className="h-11 min-w-0 rounded-xl border bg-card px-3 text-base" value={plans.some(plan => plan.id === draft.id) ? draft.id : ""} onChange={event => { const plan = plans.find(value => value.id === event.target.value); if (plan) setDraft(clonePlan(plan)); }}><option value="">Ny, osparad plan</option>{plans.map(plan => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select><Button type="button" variant="outline" className="h-11 px-3" onClick={() => setDraft(emptyPlan())}><Plus/> Ny</Button></div>
    ...
    <div className="mb-4 grid grid-cols-[1fr_auto] gap-2"><select aria-label="Lägg till befintlig peptid" className="h-11 min-w-0 rounded-xl border bg-card px-3 text-base" value="" onChange={event => addExisting(event.target.value)}><option value="">Lägg till från Peptider…</option>{peptides.filter(peptide => !peptide.archived).map(peptide => <option key={peptide.id} value={peptide.id}>{peptide.name}</option>)}</select><Button type="button" variant="outline" className="h-11 px-3" onClick={() => setDraft(plan => ({ ...plan, items: [...plan.items, emptyItem()] }))}><Plus/> Tom rad</Button></div>
    <div className="space-y-4">{draft.items.map((item, index) => {
      ...
      return <CardBlock key={item.id} className="overflow-hidden"><div className="flex items-center gap-2 border-b border-border px-4 py-3"><span className="font-mono text-xs text-muted-foreground">{String(index + 1).padStart(2, "0")}</span><Input aria-label="Peptidnamn" className="h-11 flex-1 border-0 bg-transparent px-2 text-base font-medium shadow-none focus-visible:ring-0" value={item.name} onChange={event => setItem(item.id, { name: event.target.value })} placeholder="Peptidnamn"/><Button type="button" variant="ghost" size="icon" className="size-11 text-muted-foreground" onClick={() => setDraft(plan => ({ ...plan, items: plan.items.filter(value => value.id !== item.id) }))} aria-label={`Ta bort ${item.name || "rad"}`}><Trash2/></Button></div><div className="grid grid-cols-2 gap-3 p-4">
        ...
        <label className="text-xs text-muted-foreground">Enhet<select className="mt-1.5 h-11 w-full rounded-lg border bg-background px-3 text-base" value={item.doseEntryUnit} onChange={event => setItem(item.id, { doseEntryUnit: event.target.value === "mg" ? "mg" : "mcg" })}><option value="mcg">mcg</option><option value="mg">mg</option></select></label>
        ...
        <label className="col-span-2 text-xs text-muted-foreground">Frekvens<select className="mt-1.5 h-11 w-full rounded-lg border bg-background px-3 text-base" value={item.frequency} onChange={event => setItem(item.id, { frequency: event.target.value as PurchasePlanItem["frequency"] })}><option value="daily">Varje dag</option><option value="every_n_days">Var N:e dag</option><option value="times_per_week">X dagar per vecka</option></select></label>
...
function CardBlock({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-[18px] bg-card ${className}`}>{children}</div>;
}
