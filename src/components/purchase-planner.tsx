...
function emptyItem(): PurchasePlanItem {
  return { id: uid(), name: "", route: "subcutaneous", vialMg: 10, doseMcg: 100, doseEntryUnit: "mcg", frequency: "daily", everyNDays: 2, timesPerWeek: 2, bacWaterMl: 2 };
}
...
async function exportPlanImage(plan: PurchasePlan) {
  ...
  validItems.forEach(item => {
    ...
    context.fillText(`Dos: ${doseLabel(item)}${!item.route || item.route === "subcutaneous" ? ` · ${n(planDoseIu(item))} IU` : ""}`, 72, y + 42, 470);
...
export function PurchasePlanner({ peptides, plans, onChange, onBack }: { peptides: Peptide[]; plans: PurchasePlan[]; onChange: (plans: PurchasePlan[]) => void; onBack: () => void }) {
  ...
  const addExisting = (id: string) => {
    ...
    if (!peptide) return;
    setDraft(plan => ({ ...plan, items: [...plan.items, { ...emptyItem(), name: peptide.name, route: peptide.route, vialMg: peptide.vialMg, doseMcg: peptide.doseMcg, bacWaterMl: peptide.waterMl, frequency: peptide.frequency === "every_n_days" ? "every_n_days" : peptide.frequency === "weekdays" ? "times_per_week" : "daily", everyNDays: peptide.everyNDays ?? 2, timesPerWeek: Math.max(1, peptide.weekdays.length || 1) }] }));
  };
  const save = () => {
  ...
  return <>
    ...
    <div className="space-y-4">{draft.items.map((item, index) => {
      ...
      return <CardBlock key={item.id} className="overflow-hidden"><div className="flex items-center gap-2 border-b border-border px-4 py-3"><span className="font-mono text-xs text-muted-foreground">{String(index + 1).padStart(2, "0")}</span><Input aria-label="Peptidnamn" className="h-11 flex-1 border-0 bg-transparent px-2 text-base font-medium shadow-none focus-visible:ring-0" value={item.name} onChange={event => setItem(item.id, { name: event.target.value })} placeholder="Peptidnamn"/><Button type="button" variant="ghost" size="icon" className="size-11 text-muted-foreground" onClick={() => setDraft(plan => ({ ...plan, items: plan.items.filter(value => value.id !== item.id) }))} aria-label={`Ta bort ${item.name || "rad"}`}><Trash2/></Button></div><div className="grid grid-cols-2 gap-3 p-4">
        ...
        <label className="col-span-2 text-xs text-muted-foreground">Administrering<select className="mt-1.5 h-11 w-full rounded-lg border bg-background px-3" value={item.route ?? "subcutaneous"} onChange={event => setItem(item.id, { route: event.target.value as Peptide["route"] })}><option value="subcutaneous">Subkutan</option><option value="intranasal">Intranasal</option><option value="oral">Oral</option><option value="topical">Topikal</option></select></label>
...
function Projection({ label, vials, bac, doses }: { label: string; vials: number; bac: number; doses: number }) {
  return <div className="p-4"><p className="font-mono text-xs uppercase tracking-wider text-muted-foreground">{label}</p><p className="mt-1.5 text-lg font-semibold tabular-nums">{vials} vial{vials === 1 ? "" : "er"}</p><p className="mt-1 text-xs text-muted-foreground">{doses} doser · {n(bac)} ml BAC</p></div>;
}
