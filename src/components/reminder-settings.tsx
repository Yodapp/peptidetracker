...
export function ReminderSettings({ available }: { available: boolean }) {
  ...
  const status = loading ? "Kontrollerar…" : !available ? "Logga in och synka kontot först" : !configured || !deliveryReady ? "Leveransen är inte redo ännu" : active ? "Aktiva på den här telefonen" : permission === "denied" ? "Blockerade i telefonens inställningar" : preferences.enabled ? "Aktivera på den här telefonen" : "Avstängda";
  return <Surface className="overflow-hidden">
    <div className="flex min-h-16 items-center justify-between gap-3 p-4">
      ...
      <Switch aria-label="Aktivera påminnelser" checked={active} disabled={loading || busy || !available || !configured || !deliveryReady || (ios && !installed)} onCheckedChange={checked => void save({ ...preferences, enabled: checked })}/>
    </div>
