...
export function SettingsView({ store, update, syncState, retrySync, syncError, userEmail, userId }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; syncState: "local"|"syncing"|"synced"|"error"; retrySync: () => void; syncError: string | null; userEmail?: string; userId?: string | null }) {
  ...
  return <><PageHeader eyebrow="Peptime" title="Inställningar"/>
    <div className="space-y-7">
      ...
      <section><SectionHeading title="Data"/><Surface className="grid grid-cols-2 gap-2 p-3"><Button variant="outline" onClick={()=>download("csv")}><Download/>CSV</Button><Button variant="outline" onClick={()=>download("json")}><Download/>JSON</Button></Surface><p className="mt-2 px-1 text-xs leading-5 text-muted-foreground">En lokal kopia sparas okrypterad i webbläsaren för offline-användning. Den rensas när du loggar ut.</p></section>
