...
import { ArrowLeft, Check, Clipboard, Plus, Save, Trash2 } from "lucide-react";
...
const fromRow = (row: Record<string, unknown>): Saved => ({ id: String(row.id), code: String(row.code), name: String(row.name), items: row.items as Item[], groups: (row.groups ?? []) as MixGroupSchedule[], createdAt: String(row.created_at), updatedAt: String(row.updated_at) });
...
export function ScheduleSharing({ store, update, onBack }: { store: PeptimeStore; update: React.Dispatch<React.SetStateAction<PeptimeStore>>; onBack: () => void }) {
  ...
  const lookup = async () => { setBusy(true); setError(""); setPreview(null); const result = await client.rpc("get_shared_schedule", { p_code: importCode.trim().toUpperCase() }); const value = result.data as Preview | null; if (result.error || !value || !Array.isArray(value.items)) setError("Koden finns inte."); else setPreview(value); setBusy(false); };
  ...
  const approve = () => {
    ...
    const peptides: Peptide[] = addedItems.map(item => ({ ...item, id: crypto.randomUUID(), name: item.name.trim(), shortCode: item.shortCode || item.name.slice(0, 3), mixGroupId: item.mixGroupId ? groupNames.get(key(item.mixGroupId)) ?? item.mixGroupId : undefined, remainingMg: item.vialMg, notes: "", archived: false, example: false }));
