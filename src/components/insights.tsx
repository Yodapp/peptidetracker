...
const tagLabels: Record<DailyTagId, string> = { great_sleep: "Sov bra", flushing: "Rodnad", high_energy: "Hög energi", headache: "Huvudvärk", site_soreness: "Öm injektionsplats" };
...
export function InsightsView({ store, onOpenPeptide }: { store: PeptimeStore; onOpenPeptide: (id: string) => void }) {
  ...
  const tagCounts = Object.keys(tagLabels).map(tag => ({ id: tag as DailyTagId, count: days.filter(date => notes.get(date)?.tags.includes(tag as DailyTagId)).length }));
