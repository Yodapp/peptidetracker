...
export function WellbeingInsights({ store }: { store: PeptimeStore }) {
  ...
  return <section className="mb-5 rounded-[20px] border border-border bg-card p-5 shadow-[0_16px_50px_rgba(0,0,0,.12)]">
    <div className="mb-4 flex items-center gap-2"><Activity className="size-4 text-primary"/><div><h2 className="text-lg font-medium">Mående och aktivitet</h2><p className="mt-0.5 text-xs text-muted-foreground">Dina kvällskollar över tid</p></div></div>
    ...
    <div className="mt-3 flex flex-wrap gap-2">{([7,30,90] as const).map(value => <button type="button" key={value} aria-pressed={period === value} onClick={() => setPeriod(value)} className={`min-h-9 rounded-full border px-3 text-xs ${period === value ? "border-primary bg-accent text-accent-foreground" : "border-border text-muted-foreground"}`}>{value} dagar</button>)}</div>
    ...
    <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-muted/50 p-3"><p className="text-[10px] text-muted-foreground">Genomsnitt</p><p className="mt-1 text-xl font-semibold tabular-nums">{currentAverage === null ? "–" : currentAverage.toLocaleString("sv-SE", {maximumFractionDigits:1})}</p></div><div className="rounded-xl bg-muted/50 p-3"><p className="text-[10px] text-muted-foreground">Mot föregående period</p><p className="mt-1 text-xl font-semibold tabular-nums">{change === null ? "–" : `${change > 0 ? "+" : ""}${change.toLocaleString("sv-SE", {maximumFractionDigits:1})}`}</p></div></div>
    {points.length === 0 ? <p className="grid h-48 place-items-center text-center text-sm text-muted-foreground">Fyll i kvällskollen för att se en kurva här.</p> : <svg viewBox="0 0 330 196" role="img" aria-label={`${metric.label} under ${period} dagar med doseringstillfällen markerade`} className="mt-4 w-full overflow-visible">
    ...
    <div className="mt-3 flex justify-between text-[10px] text-muted-foreground"><span>1 · {metric.low}</span><span>5 · {metric.high}</span></div>
