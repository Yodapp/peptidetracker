...
export function WellbeingInsights({ store, period, peptideId }: { store: PeptimeStore; period: InsightPeriod; peptideId?: string }) {
  ...
  return <section className="mb-5 rounded-[20px] border border-border bg-card p-5 shadow-[0_1px_2px_rgba(0,0,0,.025)]">
    ...
    {!peptideId && <label className="mt-4 block text-xs text-muted-foreground">Peptid<select value={selectedPeptideId} onChange={event => { setSelectedPeptideId(event.target.value); setSelectedDate(null); }} className="mt-2 h-11 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground"><option value="all">Alla doser</option>{store.peptides.filter(peptide => !peptide.archived).map(peptide => <option key={peptide.id} value={peptide.id}>{peptide.name}</option>)}</select></label>}
